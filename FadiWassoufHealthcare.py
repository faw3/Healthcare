...
# Creating plotly map
  ...
  with col7:
    ...
    hc.info_card(title="Household Exp.",bar_value=df["Income_composition_of resources"].mean(),content=round(df["Income_composition_of resources"].mean()),sentiment="good")
...
if menu_id == "Monetary":
    ...
    col6.metric(label = "Household Exp. On Health",value=round(df["Income_composition_of resources"].mean()),delta = "-1.5") 
...
if menu_id == "Life Expectancy Prediction":
  ...
  with col18:
    tt_inpt=st.slider(
    "Percentage Expenditure", min_value=0, max_value=1100,step=10,value=500)
